...
def adjust_interval(interval, flank_size):
    ...
    try:
        ...
        if interval_length < 101:
            ...
            return f"{adjusted_interval_str}"
        else:
            return f"{interval.chrom}\t{interval.start}\t{interval.end}\tpositive_{count}\t{interval.score}\t{interval.strand}"
